...
func getMapByKey(conn redis.Conn, key string) (map[string]string, errors.EdgeX) {
	...
	if err == redis.ErrNil {
		return keyMap, errors.NewCommonEdgeX(errors.KindEntityDoesNotExist, fmt.Sprintf("fail to query key %s in the database", key), err)
	} else if err != nil {
		return keyMap, errors.NewCommonEdgeX(errors.KindDatabaseError, fmt.Sprintf("query key %s from the database failed", key), err)
	}
	return keyMap, nil
}

// getObjectsByKeyPrefix get the value(s) stored in the specified key or keys with the same prefix
func getObjectsByKeyPrefix(conn redis.Conn, key string, keyOnly bool, isRaw bool) ([]kpModels.KVResponse, errors.EdgeX) {
	var configResp []kpModels.KVResponse
...
func getObjectsByKey(conn redis.Conn, key string, keyOnly bool, isRaw bool) ([]kpModels.KVResponse, errors.EdgeX) {
	var configResp []kpModels.KVResponse
	...
	case String:
		...
		if keyOnly {
			...
			resp = (*kpModels.KeyOnly)(&queryKey)
		} else {
			var data kpModels.StoredData
			...
			if isRaw {
				...
				if value, ok := data.Value.(string); ok {
					...
					if err != nil {
						return configResp, errors.NewCommonEdgeX(errors.KindDatabaseError, fmt.Sprintf("decode value of key %s failed", key), err)
					}
		...
		return []kpModels.KVResponse{resp}, nil
	case Hash:
