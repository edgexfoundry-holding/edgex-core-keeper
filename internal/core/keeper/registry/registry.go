...
func (r *Registry) Register(registration kpModels.Registration) {
...
type healthCheckRunner struct {
	...
	registry kpModels.Registration
	dic      *di.Container
}

func newHealthCheckRunner(r kpModels.Registration, dic *di.Container) *healthCheckRunner {
...
preServiceUpLoop:
	for {
		...
		case <-preSvcUpTicker.C:
			...
			h.registry.LastConnected = utils.MakeTimestamp()
...
func healthCheck(r kpModels.Registration, lc logger.LoggingClient, timeout time.Duration) string {
	client := http.Client{
		Timeout: timeout,
	}
